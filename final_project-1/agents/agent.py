...
class GTOAgent(BasePokerPlayer):
    def __init__(self):
        ...
        self.postflop_engine = PostflopDecisionEngine(raise_threshold=0.60, fold_threshold=0.25)
    ...
    def declare_action(self, valid_actions, hole_card, round_state):
        ...
        if self.bluff_engine.decide_bluff(adjusted_win_rate):
            ...
            raise_amount = min(max(raise_amount, raise_act_min), raise_act_max)
            self.round_stats["raise_count"] += 1
            self.opponent_stats["your_raises"] += 1
            return "raise", raise_amount

        if adjusted_win_rate < self.postflop_engine.fold_threshold:
            self.round_stats["fold_count"] += 1
            return "fold", 0
        elif adjusted_win_rate > self.postflop_engine.raise_threshold:
            raise_amount = int(raise_act_min + adjusted_win_rate * (raise_act_max - raise_act_min))
            raise_amount = min(max(raise_amount, raise_act_min), raise_act_max)
            self.round_stats["raise_count"] += 1
            self.opponent_stats["your_raises"] += 1
            return "raise", raise_amount
